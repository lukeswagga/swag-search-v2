...
class EndingSoonScraper(YahooScraperBase):
    ...
    async def scrape_ending_soon_page_async(self, keyword, page, brand_info):
        """Async version: Scrape a single page for ending soon auctions"""
        try:
    ...
    def scrape_ending_soon_page(self, keyword, page, brand_info):
        """Sync version: Scrape a single page for ending soon auctions (backward compatibility)"""
        try:
            # Build URL for auctions only, sorted by ending soon
            url = self.build_search_url(
                keyword=keyword,
    ...
    async def scrape_brand_ending_soon_async(self, brand, brand_info):
        """Async version: Scrape all ending soon auctions for a specific brand with rate-limited page requests"""
        all_listings = []
        ...
        for page in range(1, max_pages + 1):
            listings, should_continue = await self.scrape_ending_soon_page_async(
    ...
    def scrape_brand_ending_soon(self, brand, brand_info):
        """Sync version: Scrape all ending soon auctions for a specific brand (backward compatibility)"""
        all_listings = []
    ...
    async def run_ending_soon_cycle_async(self):
        """Async version: Run a complete ending soon scraping cycle"""
        cycle_start = time.time()
        self.cycle_count += 1
        
        print(f"\n🏃‍♂️ Starting ending soon cycle #{self.cycle_count} (ASYNC)")
        ...
        for brand, brand_info in self.brand_data.items():
            try:
                ...
                listings = await self.scrape_brand_ending_soon_async(brand, brand_info)
        ...
        print(f"\n📊 Ending Soon Cycle #{self.cycle_count} Complete (ASYNC):")
    ...
    def run_ending_soon_cycle(self):
        """Sync wrapper: Run a complete ending soon scraping cycle (for Railway deployment)"""
        asyncio.run(self.run_ending_soon_cycle_async())
