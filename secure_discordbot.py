...
def translate_japanese_to_english(text):
    """
    Translate Japanese text to English for American customers.
    ...
    try:
        # Translate Japanese to English using GoogleTranslator (free)
...
def create_enhanced_listing_embed(listing_data):
    ...
    # Translate title from Japanese to English for American customers
    translated_title = translate_japanese_to_english(title)
...
def determine_target_channel(price_usd, brand, deal_quality, title, scraper_source=None):
    ...
    if brand and brand in BRAND_CHANNEL_MAP:
        ...
        print(f"🏷️ Brand routing: '{brand}' -> {brand_channel}")
    elif brand and brand != "Unknown":
        print(f"⚠️ WARNING: Brand '{brand}' not found in BRAND_CHANNEL_MAP! Available brands: {list(BRAND_CHANNEL_MAP.keys())[:5]}...")
...
async def send_single_listing_enhanced(auction_data):
    ...
    try:
        ...
        print(f"📍 Target channels for '{brand}' listing: {target_channels}")
...
async def send_to_premium_channels_immediately(listing_data):
    ...
    try:
        ...
        # Determine target channel based on brand using BRAND_CHANNEL_MAP
        channel = None
        if brand and brand in BRAND_CHANNEL_MAP:
            brand_channel_name = f"🏷️-{BRAND_CHANNEL_MAP[brand]}"
            channel = discord.utils.get(guild.text_channels, name=brand_channel_name)
            print(f"🔍 Looking for brand channel: {brand_channel_name} for brand: {brand}")
        else:
            print(f"⚠️ Brand '{brand}' not found in BRAND_CHANNEL_MAP")
...
async def send_guide_command(ctx):
    ...
    try:
        ...
        await ctx.send(f"📤 Sending updated instructions to #{channel.name}...")
        
        # Intro message
        intro_text = """🎌 **Welcome to SwagSearch — Japan → Discord Alerts**

We track Yahoo Japan in real time and send designer & archive listings directly into this server.

**Sign up for Premium Tiers:** https://whop.com/swagsearch/

**Start here:**"""
        
        # Section 1
        section1_text = """**1️⃣ Free Tier — Browse Japan Listings (Starter Mode)**

If you're Free Tier, you get:

✔ **Daily Digest at 9AM EST**
A curated post of the day's top deals.

✔ **Standard Feed (5 listings/hour)**
A rotating preview of real listings pulled from our scrapers.

→ Go to #daily-digest
→ Go to #standard-feed"""
        
        # Section 2
        section2_text = """**2️⃣ Standard Tier — Full Japan Alert System**

Standard unlocks all 5 real-time channels inside the Find Alerts Category:

✔ **Auction Alerts** (unified feed)
...
These run continuously all day.

→ Go to #auction-alerts
→ Explore Find Alerts Category"""
        
        # Section 3
        section3_text = """**3️⃣ Instant Tier — Designer Channels (22 Total)**

Instant includes everything above plus all designer-specific channels:

Raf Simons · Rick Owens · Margiela · Junya · CDG
Undercover · Yohji · Kiko · Kapital · +14 more

Ideal for collectors & resellers who only want alerts from specific designers.

→ Go to Brand Channels Category"""
        
        # Section 4
        section4_text = """**4️⃣ How Buying Works (No Japan Address Needed)**

Every alert includes proxy buying links (Buyee / ZenMarket).
Click, pay, and ship internationally with no setup needed.

**📦 Buyee**
Official Yahoo Auctions partner with easy-to-use interface.
• Official Yahoo Auctions integration
• English support
• Package consolidation available
• International shipping worldwide

**Sign up:** https://buyee.jp/

**🛒 ZenMarket**
Popular choice with competitive fees and great customer service.
• Full English support
• Detailed guides and tutorials
• Transparent fee structure
• Package consolidation available

**Sign up:** https://zenmarket.jp/

**How to Use:**
1. Click the Buyee or ZenMarket link in any listing
2. Sign up for an account (takes 2 minutes)
3. Add the item to your cart and checkout
4. The proxy will bid/buy on your behalf
5. Once won, pay for the item + proxy fees + shipping
6. Item ships to your address internationally"""
        
        # Section 5
        section5_text = """**5️⃣ Need Help?**

Ask in #support
Or DM the bot "help" for quick commands."""
        
        # Send all messages with delays
        messages = []
        texts = [intro_text, section1_text, section2_text, section3_text, section4_text, section5_text]
        ...
        for i, text in enumerate(texts):
            try:
                ...
                if i < len(texts) - 1:  # Don't sleep after last message
        ...
        if messages:
            try:
                ...
                await ctx.send(f"✅ Posted and pinned {len(messages)} messages to #{channel.name}")
            except discord.errors.Forbidden:
                await ctx.send(f"✅ Posted {len(messages)} messages to #{channel.name} (could not pin - need 'Manage Messages' permission)")
            except discord.errors.HTTPException:
                await ctx.send(f"✅ Posted {len(messages)} messages to #{channel.name} (could not pin - channel may have too many pinned messages)")
