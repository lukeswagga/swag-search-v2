...
class BudgetStealsScraper(YahooScraperBase):
    ...
    async def scrape_budget_page_async(self, keyword, page, brand_info):
        """Async version: Scrape a single page for budget items"""
        try:
    ...
    def scrape_budget_page(self, keyword, page, brand_info):
        """Sync version: Scrape a single page for budget items (backward compatibility)"""
        try:
            # Build URL for all items (auctions + fixed price), sorted by price ascending
            url = self.build_search_url(
                keyword=keyword,
    ...
    async def scrape_brand_budget_steals_async(self, brand, brand_info):
        """Async version: Scrape all budget steals for a specific brand with rate-limited page requests"""
        all_listings = []
        ...
        # Determine max pages (after first few cycles, only check first page)
        max_pages = 1 if self.cycle_count > 3 else min(5, self.max_pages_per_brand)

        # REDUCED CONCURRENCY: Process pages sequentially to avoid rate limiting
        for page in range(1, max_pages + 1):
            listings, should_continue = await self.scrape_budget_page_async(
    ...
    def scrape_brand_budget_steals(self, brand, brand_info):
        """Sync version: Scrape all budget steals for a specific brand (backward compatibility)"""
        all_listings = []
    ...
    async def run_budget_steals_cycle_async(self):
        """Async version: Run a complete budget steals scraping cycle"""
        cycle_start = time.time()
        self.cycle_count += 1
        
        print(f"\n💰 Starting budget steals cycle #{self.cycle_count} (ASYNC)")
        ...
        for brand, brand_info in self.brand_data.items():
            try:
                ...
                listings = await self.scrape_brand_budget_steals_async(brand, brand_info)
        ...
        print(f"\n📊 Budget Steals Cycle #{self.cycle_count} Complete (ASYNC):")
    ...
    def run_budget_steals_cycle(self):
        """Sync wrapper: Run a complete budget steals scraping cycle (for Railway deployment)"""
        asyncio.run(self.run_budget_steals_cycle_async())
