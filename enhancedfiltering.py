...
class EnhancedSpamDetector:
    def __init__(self):
        ...
        # Enhanced exclusion terms - FOCUSED on non-fashion items only
        self.excluded_terms = [
            # NEW EXCLUSIONS
            'lego', 'レゴ', 'water tank', 'ウォータータンク', '水タンク',
            'bmw touring', 'bmw e91', 'mazda', 'マツダ',

            # Vehicles & Parts - SPECIFIC ONLY (removed 'car', 'parts', 'tank' - too broad)
            'automobile', 'motorcycle', 'motorbike', 'scooter', 'バイク', 'モーターサイクル',
            'truck', 'vehicle', '自動車', 'トラック',
            'car engine', 'エンジン', 'car wheels', 'タイヤ', 'car exhaust',
            'cb400', 'vtr250', 'cbx', 'undercowl',
            'jdirectitems auction → automobile', 'jdirectitems auction → motorcycle',

            # Specific car parts - MUST have car context
            'bumper', 'バンパー', 'spoiler', 'スポイラー',
            'wheel rim', 'グリル', 'fender',
            'headlight', 'テールライト', 'taillight',
            'suspension', 'ブレーキ brake',
            'exhaust system', 'turbo ターボ',
            'car navigation', 'シートカバー seat cover',

            # Home/Industrial items - SPECIFIC ONLY
            'industrial', '工業用', 'equipment', '機器',

            # Toys & Games
            'toy', 'おもちゃ', 'figure', 'フィギュア',
            'doll', 'ドール', 'puzzle', 'パズル', 'pokemon', 'ポケモン',
            'trading card', 'トレーディングカード',
            'model kit', 'プラモデル',

            # Electronics - BE SPECIFIC (removed 'game', 'computer', 'pc' - used in fashion)
            'smartphone', 'iphone case', 'android', 'tablet',
            'スマートフォン', 'タブレット', 'laptop',
            'server', 'motherboard', 'graphics card', 'ram memory', 'ssd',

            # Food & Consumables
            'food', '食品', 'drink', '飲料', 'supplement',
            'medicine', '薬', 'snack', 'お菓子',

            # Sports Equipment - SPECIFIC ITEMS ONLY
            'bicycle', '自転車', 'golf club', 'fishing rod', '釣り',
            'tennis racket', 'baseball bat', '野球',

            # Home Goods
            'furniture', '家具', 'appliance', '家電', 'kitchen',
            'bedding', '寝具', 'curtain', 'カーテン',

            # Obvious replicas/fakes
            'fake', 'knock off', 'bootleg', 'unauthorized',
            'スーパーコピー', '偽物',

            # Non-clothing items
            'book', 'magazine', 'cd album', 'dvd', 'poster',
            '本', '雑誌', 'ポスター',

            # REMOVED: 'ダメージ' (damage) - common in fashion for distressed items
            # REMOVED: 'car' - blocks sneakers/cardigan/parka
            # REMOVED: 'tank' - blocks tank tops
            # REMOVED: 'replica' - some legit brands use this (Maison Margiela Replica line)
            # REMOVED: 'game' - used in fashion (Comme des Garcons PLAY)
            # REMOVED: 'parts' - too broad

            # Parts only - BE SPECIFIC
            'parts only', '部品のみ', 'badge only', 'patch only',

            # Obvious non-fashion with context
            'phone case', 'スマホ', '携帯電話', 'パソコン'
        ]
