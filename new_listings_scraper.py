...
class NewListingsScraper(YahooScraperBase):
    ...
    async def scrape_new_listings_page_async(self, keyword, page, brand_info):
        """Async version: Scrape a single page for new auction listings"""
        try:
    ...
    def scrape_new_listings_page(self, keyword, page, brand_info):
        """Sync version: Scrape a single page for new auction listings (backward compatibility)"""
        try:
            # Build URL for auctions only, sorted by newest
            url = self.build_search_url(
                keyword=keyword,
    ...
    async def scrape_brand_new_listings_async(self, brand, brand_info):
        """Async version: Scrape all new listings for a specific brand with rate-limited page requests"""
        all_listings = []
        ...
        # Yahoo returns 500 errors when hit too hard
        for page in range(1, max_pages + 1):
            listings, should_continue = await self.scrape_new_listings_page_async(
    ...
    def scrape_brand_new_listings(self, brand, brand_info):
        """Sync version: Scrape all new listings for a specific brand (backward compatibility)"""
        all_listings = []
    ...
    async def run_new_listings_cycle_async(self):
        """Async version: Run a complete new listings scraping cycle"""
        cycle_start = time.time()
        self.cycle_count += 1
        current_time = datetime.now(timezone.utc)
        
        print(f"\n🆕 Starting new listings cycle #{self.cycle_count} (ASYNC)")
        ...
        # This prevents HTTP 500 errors from rate limiting
        all_brand_listings = []
        ...
        for brand, brand_info in self.brand_data.items():
            try:
                ...
                listings = await self.scrape_brand_new_listings_async(brand, brand_info)
        ...
        print(f"\n📊 New Listings Cycle #{self.cycle_count} Complete (ASYNC):")
    ...
    def run_new_listings_cycle(self):
        """Sync wrapper: Run a complete new listings scraping cycle (for Railway deployment)"""
        asyncio.run(self.run_new_listings_cycle_async())
